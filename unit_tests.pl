...
sub run_get_tests {
	...
	is(get_for_statement_postexec('for($i=0; $i < $x; $i++)'), '$i++',            'Get For Statement Post Execution');
	is(get_function_args('sub function_name ( $$ ) { '),       '(arg0, arg1)',    'Get Function Arguments');
	is(get_function_name('sub function_name ( $$ ) { '),       'function_name',          'Get Function Name');
	is(get_if_condition('if (true) {'),                        'true',          'Get ');
	is(get_if_routine('if (true) { routine };'),               'routine',          'Get ');
	is(get_incdec_op('--$i'),                                  '-',          'Get ');
	is(get_post_var('$i++'),                                   '$i',          'Get ');
	is(get_pre_var('++$i'),                                    '$i',          'Get ');
	is(get_print('print $i."\n"'),                             '$i."\n"',          'Get ');
	is(get_while_condition('while (true) {'),                  'true',          'Get ');
...
sub run_strip_tests {
	...
	is(strip_invalid_python('@ARGV'),                  'sys.argv[1:0]',         'Strip Invalid Python');
